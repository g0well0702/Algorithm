...
int counting(int arr[]) {
	...
	for (int i = 1; i < 10; i++) {
		if (max < arr[i]) { // max 갱신 조건, 같은 때는 갱신할 필요가 없다. 여기서 인덱스가 6일 때와 아닐 때를 나눠줘야 한다
            if(i == 6) max = ceil(arr[i] / 2.0); // 만약 i가 6이면 9도 같이 카운팅 된것이므로 2로 나눠주고 값을 반올림(ceil은 올림이긴 한데 큰 상관이 없음, 어차피 0.5 로 나올테니까 ex)만약 696 이라면 두 개가 필요, 6에 3이 저장되어있을 것이므로 2.0으로 나눠주고 반올림)
